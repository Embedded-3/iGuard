...
// 함수 프로토타입
static uint8 select_fan_speed(uint16 co2, double humi, double temp);
...
uint16 havc_control(Hvac* hvac, const Sensor_Data data) {
    switch(hvac->mode){
        case EXT_MODE:
            ...
            //fan_control(hvac->speed);  // 팬 속도 제어 함수 호출
            break;
...
uint8 select_fan_speed(uint16 co2, double humi, double temp){
